...
export function Libraries({ user }: { user: User }) {
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    try { const result = await api<{ id: string }>('/libraries', { method: 'POST', idempotencyKey: requestKey, body: { name: data.get('name'), mode: data.get('mode'), manager_user_id: user.id } }); setSelected(result.id); setCreating(false); libraries.reload(); setRequestKey(crypto.randomUUID()); }
